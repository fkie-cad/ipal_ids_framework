...
class incStat:
    def __init__(
        self, Lambda, ID, init_time=0, isTypeDiff=False
        ...
        self.covs = (
            []
        )  # a list of incStat_covs (references) with relate to this incStat
...
class incStat_cov:
    ...
    def update_cov(
        ...
        if ID == self.incStats[0].ID:
            ...
            settings.logger.info("update_cov ID error")
            return  # error
...
class incStatDB:
    ...
    def update_get_1D_Stats(
        self, ID, t, v, Lambda=1, isTypeDiff=False
    ):  # weight, mean, std
    ...
    def update_get_2D_Stats(
        self, ID1, ID2, t1, v1, Lambda=1, level=1
    ...
    ):  # IDs is a 2-element list or tuple
        # Default Lambda?
        Lambda = self.get_lambda(Lambda)
        if IDs is None:
            IDs = [0, 1]
